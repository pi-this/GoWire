...
yes_bt = None
no_bt = None
op = "start" # option status current
...
def placeCheck():
    ...
    global dayTime, yes, yes_bt, no_bt, op
    ...
    if place == "center":
        ...
        use(False)
        no_bt.destroy()
        yes_bt.config(text=" 🔍 Find a robot")
...
def special1_center(robot):
    ...
    global heat, yes_bt, yes, money, ai
...
Center Square: Find Help
    ...
    Captain, your prowess as a formidable and inspiring leader is undeniable. Yet, the weight of your responsibilities is immense, and you cannot shoulder it all alone. Recognizing this truth was a wise decision, and it speaks to your strength in seeking assistance. Fortunately, fate has brought you a companion—a fellow robot, ready to lend its expertise. Would you like to hear the insights it has to offer?
        ...
        yes_bt.config(text="👂 Listen to robot")
        op = "center find help"

        if yes and op == "center find help": # if you listen to the robot then you will hear
            addHeat(1)
            update()
            "story scroll info"
            # Create a scrolled text widget
            ...
            # Insert some example text
            text_info = f"""
Center Square: Find help : Listen to robot

    In the dim light of a rusted alleyway, a sleek robot stood, its intricate circuitry humming softly.

“Head North,” it advised, its voice smooth and confident. “There lies a city bustling with activity, a place where many robots gather.”

“The market is vast,” it explained, optics glowing with enthusiasm. “Filled with high prices, yes, but if you possess something valuable, you may negotiate a good deal. Traders are always looking for rare items.”

“The city is a hub of knowledge and resources. If you seek to enhance your capabilities, it is the place to be.”

The promise of discovery and opportunity lay just beyond the horizon, and the city awaited, ready to unveil its secrets.
            """
...
You consider your options carefully, knowing that every decision in this game can lead to new opportunities or unforeseen consequences. What will you choose?
            ...
            # Make the text widget read-only
            scroll_text.config(state=tk.DISABLED)
            yes_bt.config(text="DEAL!")
            
            if yes and op == "center deal":
                #DEAL!
                ai = -100
                update()
